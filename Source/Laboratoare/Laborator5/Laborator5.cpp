...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_C)) {
			widthOrtho += deltaTime * cameraSpeed * 10;
			projectionMatrix = glm::ortho(-widthOrtho / 2, widthOrtho / 2, -heightOrtho / 2, heightOrtho, zNear, zFar);
		}
		...
		if (window->KeyHold(GLFW_KEY_B)) {
			heightOrtho += deltaTime * cameraSpeed * 10;
			projectionMatrix = glm::ortho(-widthOrtho / 2, widthOrtho / 2, -heightOrtho / 2, heightOrtho, zNear, zFar);
		}
