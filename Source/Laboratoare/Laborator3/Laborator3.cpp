...
int i = 150;
int trans = 70;
int tog = 0, togScale = 0;
int range = trans;
int rangeScale = trans;
float sqRotation = 0.8f;
float scaleX = 400, scaleY = 250;

void Laborator3::Update(float deltaTimeSeconds)
{
	// TODO: update steps for translation, rotation, scale, in order to create animations
	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(i, 250);
	...
	modelMatrix *= Transform2D::Translate(50, 50);
	sqRotation += deltaTimeSeconds;
	modelMatrix *= Transform2D::Rotate(sqRotation);
	modelMatrix *= Transform2D::Translate(-50, -50);
