...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int materialPos = glGetUniformLocation(shader->program, "material_shininess");
	glUniform1i(materialPos, materialShininess);

	int materialKdPos = glGetUniformLocation(shader->program, "material_kd");
	glUniform1f(materialKdPos, materialKd);

	int materialKsPos = glGetUniformLocation(shader->program, "material_ks");
	glUniform1f(materialKsPos, materialKs);

	int objectPos = glGetUniformLocation(shader->program, "object_color");
	glUniform3fv(objectPos, 1, glm::value_ptr(color));
